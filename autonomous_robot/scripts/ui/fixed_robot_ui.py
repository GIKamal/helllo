...
class RobotControlUI(QMainWindow):
    def __init__(self):
        ...
        # Initialize UI components and attributes first
        ...
        self.system_messages = []  # Initialize system messages
        
        self.last_goals = []
        self.max_goal_history = 5
        
        self.map_click_pos = None
        self.map_label.mousePressEvent = self.handle_map_click
        self.map_click_positions = []
    ...
    def init_sensor_status_panel(self):
        ...
        self.left_encoder.setDigitCount(8)  # FIXED: was 😎
        ...
        self.right_encoder.setDigitCount(8)  # FIXED: was 😎
    ...
    def init_visualization_panel(self):
        ...
        if RVIZ_AVAILABLE:
            ...
            # Load saved RViz configuration
            import os
            import rospkg # type: ignore
            config_file = os.path.join(rospkg.RosPack().get_path('autonomous_robot'), 'rviz', 'my_robot_config.rviz')
            self.rviz_frame.loadConfig(config_file)
            
            self.rviz_manager = self.rviz_frame.getManager()
            self.rviz_manager.setFixedFrame("map")
            
            # Ensure RobotModel display is included
            robot_display = self.rviz_manager.createDisplay("rviz/RobotModel", "RobotModel", True)
            robot_display.subProp("Robot Description").setValue("/robot_description")
    ...
    def draw_robot_on_image(self, image):
        try:
            ...
            robot_y_px = self.last_map_data.info.height - robot_y_px
            ...
            end_y = robot_y_px - int(15 * math.sin(theta))
    ...
    def init_ros_communication(self):
        ...
        rospy.Subscriber('/imu/temperature', Float32, self.imu_temperature_callback)
    ...
    def imu_callback(self, msg):
        try:
            if not hasattr(self, 'mpu_data'):
                self.mpu_data = {'accel': Vector3(), 'gyro': Vector3(), 'temp': 0.0}
                rospy.logwarn("mpu_data was not initialized, reinitializing in imu_callback")
            ...
            rospy.logerr(f"IMU callback error: {str(e)}")

    def imu_temperature_callback(self, msg):
        try:
            self.mpu_data['temp'] = msg.data
            self.imu_temp.display(f"{msg.data:.1f}")
        except Exception as e:
            rospy.logerr(f"IMU temperature callback error: {str(e)}")

    def left_rpm_callback(self, msg):
        try:
            if not hasattr(self, 'motor_data'):
                self.motor_data = {'left_rpm': 0, 'right_rpm': 0, 'left_encoder': 0, 'right_encoder': 0}
                rospy.logwarn("motor_data was not initialized, reinitializing")
            self.motor_data['left_rpm'] = msg.data
            self.left_rpm.display(str(msg.data))
        except Exception as e:
            rospy.logerr(f"Left RPM callback error: {str(e)}")
    ...
    def right_rpm_callback(self, msg):
        try:
            ...
            rospy.logerr(f"Right RPM callback error: {str(e)}")
    ...
    def left_encoder_callback(self, msg):
        try:
            ...
            rospy.logerr(f"Left encoder callback error: {str(e)}")
    ...
    def right_encoder_callback(self, msg):
        try:
            ...
            rospy.logerr(f"Right encoder callback error: {str(e)}")

    def voltage_callback(self, msg):
        try:
            self.system_data['voltage'] = msg.data
        except Exception as e:
            rospy.logerr(f"Voltage callback error: {str(e)}")

    def current_callback(self, msg):
        try:
            self.system_data['current'] = msg.data
        except Exception as e:
            rospy.logerr(f"Current callback error: {str(e)}")
    ...
    def temperature_callback(self, msg):
        try:
            ...
            rospy.logerr(f"Temperature callback error: {str(e)}")
...
if __name__ == '__main__':  # FIXED: was if _name_ == '_main_':
